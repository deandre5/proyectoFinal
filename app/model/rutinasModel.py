...
class Rutinas:
    ...
    def actualizar(self,  id, nombre, descripcion, intensidad, dificultad, categoria):
        try:
            ...
            sql = "UPDATE rutinas SET nombre = %s, descripcion = %s, intensidad = %s, dificultad = %s, categoria = %s WHERE id = %s"
...
# funcion que recibe un id de rutina para su posterior eliminacion
    ...
    def reporte(self):
        try:
            ...
            sql = "SELECT r.nombre, count(*) FROM rutinas r, personas p WHERE r.id = p.idrutina GROUP BY r.nombre ORDER BY r.nombre ASC"
